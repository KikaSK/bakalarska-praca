...
vector<Point> Mesh::get_breakers(Triangle T, const vector<Edge> &active_edges,
  ...
  for (Point vertex : _mesh_points) {
    ...
    if (dist1 < dist && vertex != T.A() && vertex != T.B() && vertex != T.C()) {
...
Mesh::empty_surrounding(Point P, const Edge working_edge, const Triangle N,
  ...
  for (Point point : _mesh_points) {
    if (point != P && point != working_edge.A() && point != working_edge.B()) {
      ...
      if (dist < min_dist &&
          is_border_point(point, active_edges, checked_edges, bounding_edges)) {
...
bool intersect(const Triangle &T1, const Triangle &T2) {
  ...
  if (Vector(T1.A(), T2.A()).get_length() > 5 * T1.AB().get_length())
    return false;
  ...
  numeric c = n1.z();
  numeric d =
      -(n1.x() * T1.A().x() + n1.y() * T1.A().y() + n1.z() * T1.A().z());
  ex plane1 = a * x + b * y + c * z + d;
  numeric valA, valB, valC;
  valA = ex_to<numeric>(
      plane1.subs(lst{x == T2.A().x(), y == T2.A().y(), z == T2.A().z()})
          .evalf());
  valB = ex_to<numeric>(
      plane1.subs(lst{x == T2.B().x(), y == T2.B().y(), z == T2.B().z()})
          .evalf());
  valC = ex_to<numeric>(
      plane1.subs(lst{x == T2.C().x(), y == T2.C().y(), z == T2.C().z()})
          .evalf());

  if (valA < -10e-8 && valB < -10e-8 && valC < -10e-8)
    return false;
  if (valA > 10e-8 && valB > 10e-8 && valC > 10e-8)
    return false;
  ...
  std::optional<Point> intersectionAB = std::nullopt,
                       intersectionBC = std::nullopt,
                       intersectionCA = std::nullopt;

  // ak A a B sú na opačnej strane, nájdeme priesečník s rovinou
  if (AB < -10e-8) {
    Point p = T2.A();
    Vector u = Vector(T2.A(), T2.B());
    numeric t = -(a * p.x() + b * p.y() + c * p.z() + d) /
                (a * u.x() + b * u.y() + c * u.z());
  ...
  assertm(!(intersectionAB.has_value() && intersectionBC.has_value() &&
            intersectionCA.has_value()),
  ...
  // mame trojuholnik T v rovine a body P1 a P2 v tej istej rovine ktore nelezia
  // v trojuholniku chceme zistit ci usecka P1, P2 pretína trojuholník T
  ...
  // usecky sa pretinaju ak sa pretina usecka1 s priamkou2 (prva cast) a usecka2
  // s priamkou1 (druha cast)
  if (
      // pretinaju sa priamka AB a usecka P1P2
      ((Vector(T1.A(), P1.value()) ^ vAB) *
       (Vector(T1.A(), P2.value()) ^ vAB)) < 0 &&
      // pretinaju sa priamka P1P1 a usecka AB
      ((Vector(P1.value(), T1.A()) ^ other) *
       (Vector(P1.value(), T1.B()) ^ other)) < 0)
    return true;

  // to iste pre usecku BC
  if (
      // pretinaju sa priamka BC a usecka P1P2
      ((Vector(T1.B(), P1.value()) ^ vBC) *
       (Vector(T1.B(), P2.value()) ^ vBC)) < 0 &&
      // pretinaju sa priamka P1P1 a usecka BC
      ((Vector(P1.value(), T1.B()) ^ other) *
       (Vector(P1.value(), T1.C()) ^ other)) < 0)
    return true;

  // to iste pre usecku CA
  if (
      // pretinaju sa priamka CA a usecka P1P2
      ((Vector(T1.C(), P1.value()) ^ vCA) *
       (Vector(T1.C(), P2.value()) ^ vCA)) < 0 &&
      // pretinaju sa priamka P1P1 a usecka CA
      ((Vector(P1.value(), T1.C()) ^ other) *
       (Vector(P1.value(), T1.A()) ^ other)) < 0)
    return true;
