...
// throws error if it is found more than once
void BasicAlgorithm::delete_from_bounding(const Edge &edge) {
  size_t counter = 0;
  std::optional<size_t> index = std::nullopt;
...
void BasicAlgorithm::push_edge_to_bounding(const Edge &edge) {

  assertm(!is_bounding(edge), "Edge already in bounding_edges!");
...
bool BasicAlgorithm::good_edges(const Edge &working_edge, const Point &P) {
  ...
  return (!((my_mesh.is_in_mesh(new_edge1) &&
           ...
           (my_mesh.is_in_mesh(new_edge2) &&
            !is_border(new_edge2))))&&(intersections_off || (!my_mesh.intersections(T)));
}
...
BasicAlgorithm::find_closest_prev_next(const Edge &working_edge, const vector<Point> &prev, const vector<Point> &next) {
...
bool BasicAlgorithm::is_vertex_good_possibility(const Point candidate, const Point prev,
  ...
  if (my_triangle.is_triangle() &&
      good_edges(working_edge, candidate)) {
...
BasicAlgorithm::empty_surrounding(Point P, const Edge working_edge, const Triangle N) const {
  numeric min_dist = 0.4 * e_size;
  vector<pair<numeric, Point>> close_points;

  for (Point point : my_mesh.mesh_points) {
    if (point != P && point != working_edge.A() && point != working_edge.B()) {
      numeric dist = Vector(point, P).get_length();
      if (dist < min_dist && is_border_point(point)) {
...
int BasicAlgorithm::update_border(const Edge &new_edge1, const Edge &new_edge2) {
  ...
  if (bounding_box.is_on(new_edge1.A()) && bounding_box.is_on(new_edge1.B())) {
    ...
    delete_from_checked(new_edge1);
    push_edge_to_bounding(new_edge1);
  }
  if (bounding_box.is_on(new_edge2.A()) && bounding_box.is_on(new_edge2.B())) {
    ...
    delete_from_checked(new_edge2);
    push_edge_to_bounding(new_edge2);
  }
...
bool BasicAlgorithm::fix_proj(const Edge &working_edge) {
  ...
  if (auto surrounding_points =
          empty_surrounding(projected, working_edge, neighbour_triangle);
  ...
  auto close_edge = get_closest_edge(working_edge.get_midpoint(), neighbour_triangle)
                        .value();
  ...
  /*if (!close_walls.empty() || !bounding_box.is_inside(projected)) {
    ...
    if (good_orientation(working_edge, new_point, neighbour_triangle) &&
        ...
        good_edges(working_edge, new_point) &&
...
void BasicAlgorithm::starting() {
  ...
  while (!active_edges.empty()) {
    ...
    working_edge = active_edges.front();
    swap(active_edges.front(), active_edges.back());
...
void BasicAlgorithm::ending() {
  ...
  while (!active_edges.empty()) {
    ...
    if (new_edges > 0) {
      ...
      checked_edges = new_checked_edges;
