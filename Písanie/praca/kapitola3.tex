...
trojuholníky v miestach s menším zakrivením, túto vlastnosť nazývame \textit{adaptivita}.
...
Tieto bunky nazývame \textit{voxely} z anglického \textit{volumetric pixel}.
Následne pomocou znamienka funkcie danej implicitne vyhodnotenej vo vrcholoch týchto \textit{voxelov}
...
Medzi najznámejšie \textit{Space docomposition} algoritmy
patrí \textit{Marching Cubes} \cite{lorensen1987marching} a \textit{Marching Tetrahedra} 
...
\begin{definition}
    ...
    $\mathbf{e}(x_i, x_j)$ a vrchola $x_{new}$. Tento trojuholník môžeme pridať do 
    ...
    v bode C, pričom C je stred opísanej kružnice trojuholníka $\mathbf{T}$. Pod rovnakou orientáciou 
...
sledovaní povrchu. Prvý krát bol prezentovaný v roku 1996 \cite{hilton1996marching}. 
...
je sledovanie povrchu dosiahnuté expandovaním hraničných hrán, pričom v každom kroku sa vytvorí 
najviac $1$ trojuholník. Kdežto E. Hartmann \cite{hartmann1998marching} v jednom kroku 
vytvára aj viac trojuholníkov. Algoritmus v každom kroku projektuje na plochu polygón vytvorený
...
Algoritmus od A. Hiltona et. al. začína v počiatočnom bode na povrchu plochy. 
Následne vytvorí prvý trojuholník na povrchu a hrany vzniknutého 
trojuholníka vloží do zoznamu hraničných hrán. 
Opakujúc sériu krokov postupne zväčšuje trianguláciu. V každom z týchto krokov zo zoznamu 
vyberie jednu hranu $E$. Táto hrana susedí práve s jedným trojuholníkom. V rovine susedného 
trojuholníka vytvorí kolmicu na hranu $E$, ako na obrázku \ref{obr:new_vertex} a následne na nej bod 
...
Následne sa tento bod pomocou numerických metód sprojektuje na povrch a overí sa 
...
trojuholník s hraničným vrcholom už existujúcej triangulácie ktorý pretína guľu kontrolujúcu 
\textit{Delaunayovu podmienku}, ak taký existuje. Ak ju však ani tento trojuholník nesplní, 
